...
export const Navbar = ({ onOpenCreateModal }: NavbarProps) => {
        ...
        <nav className="glass sticky top-0 z-50 px-6 py-4 flex items-center justify-between border-b border-white/10 shadow-lg">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <span className="text-xs block text-slate-400 font-medium tracking-tight">
                        Todo Dashboard
                    </span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-6">
                ...
                <Link
                    to="/todos"
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 ${
                        isActive('/todos') || location.pathname.startsWith('/todos/')
                            ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-500/20'
                            : 'text-slate-300 hover:text-white hover:bg-white/5'
                    }`}
                >

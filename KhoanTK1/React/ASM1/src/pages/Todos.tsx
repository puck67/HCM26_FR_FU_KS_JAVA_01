...
export const Todos = () => {
    const { state, fetchTodos, deleteTodo, toggleTodoCompletion } = useTodos();
    ...
    const removeTodoItem = async (id: string, e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const toggleStatus = async (todo: Todo, e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        try {
    ...
    const selectTodoForEdit = (todo: Todo, e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        setTodoToEdit(todo);
        setIsFormOpen(true);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">
                        Manage your assignments, track completion, and edit todos.
                    </p>
                </div>
                <button
            ...
            {loading && todos.length === 0 ? (
                <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                        <div
                            key={i}
                            className="glass-card p-5 rounded-2xl animate-pulse flex items-start gap-4"
                        >
                ...
                <>
                    {filteredTodos.length === 0 ? (
                        <div className="glass-card py-16 text-center rounded-2xl">
                            ...
                            <h3 className="text-lg font-bold text-white mb-1">No todos found</h3>
                                ...
                                >
                                    <div className="flex items-start gap-4 flex-1">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className={`text-sm max-w-xl ${todo.completed ? 'text-slate-500' : 'text-slate-300'}`}>
                                                {todo.description || <span className="italic text-slate-500">No description provided</span>}
                                            </p>
                                    ...
                                    <div className="flex items-center gap-2 border-t md:border-t-0 border-white/5 pt-3 md:pt-0 self-end md:self-center">
                                        ...
                                        <button
                                            onClick={(e) => selectTodoForEdit(todo, e)}

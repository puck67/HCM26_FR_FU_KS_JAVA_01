...
public class CourseController {
    ...
    @GetMapping
    public String listAllCourses(@RequestParam(defaultValue = "0") int page,
                                 @RequestParam(defaultValue = "10") int size,
                                 Model model) {
        log.info("Request to list all courses: page={}, size={}", page, size);
    ...
    @GetMapping("/template")
    public org.springframework.http.ResponseEntity<byte[]> downloadCourseTemplate() {
        log.info("Request to download course template");
    ...
    @GetMapping("/export")
    public org.springframework.http.ResponseEntity<byte[]> exportCoursesToCsv() {
        log.info("Request to export courses to CSV");
    ...
    @PostMapping("/import")
    public String importCoursesFromCsv(@RequestParam("file") MultipartFile file) {
        if (file.isEmpty()) {
            log.warn("Uploaded file is empty");
            return "redirect:/courses";
        }
        log.info("Request to import courses from file: {}", file.getOriginalFilename());
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (data.length >= 5) {
                    try {
                        ...
                        log.error("Error parsing date in CSV line: {}. Skipping record.", line, dtpe);
                    } catch (Exception ex) {
            ...
            if (!coursesToSave.isEmpty()) {
                ...
                log.info("Successfully imported {} course(s)", coursesToSave.size());
            }
        } catch (Exception e) {
            log.error("Failed to import courses from file", e);
        }
        return "redirect:/courses";
    }
    ...
    public String showAddCourseForm(Model model) {
        log.info("Request to show add course form");
    ...
    public String addNewCourse(@ModelAttribute Course course) {
        log.info("Request to add new course: code={}, name={}", course.getId() != null ? course.getId().getCourseCode() : "null", course.getCourseName());
        courseService.saveCourse(course);
        return "redirect:/courses";
    }

    @PostMapping("/edit/{courseCode}")
    public String updateExistingCourse(@PathVariable String courseCode, 
                                       @RequestParam("startDate") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate, 
                                       @ModelAttribute Course course) {
        log.info("Request to update course: code={}, startDate={}", courseCode, startDate);

...
public class HotelRoomService {
    ...
    public void addRoom(Scanner sc) {
        ...
        try {
            ...
            // LAYER 2 VALIDATION: Kiểm tra thực thể trước khi thực hiện thao tác DB
            if (!Validator.validateHotelRoom(room)) {
                ConsoleHelper.printError("Lỗi dữ liệu: Dữ liệu phòng không hợp lệ (Layer 2 Validation failed)!");
                return;
            }
    ...
    /** Hiển thị room hiện tại rồi lần lượt gọi từng hàm cập nhật field. */
    private void applyUpdates(Scanner sc, HotelRoom room) {
        ...
        // LAYER 2 VALIDATION: Kiểm tra thực thể trước khi thực hiện cập nhật DB
        if (!Validator.validateHotelRoom(room)) {
            ConsoleHelper.printError("Lỗi dữ liệu: Dữ liệu phòng cập nhật không hợp lệ (Layer 2 Validation failed)!");
            return;
        }
    ...
    /** Dùng switch expression (Java 14+) — trả danh sách kết quả tìm kiếm. */
    ...
    private List<HotelRoom> searchById(Scanner sc) throws SQLException {
        ...
        // findById trả Optional → chuyển thành List (0 hoặc 1 phần tử)
    ...
    /** Trả về Comparator tương ứng theo lựa chọn. */
    ...
    /**
     * Trả Optional.empty() sau 3 lần thất bại liên tiếp.
     */
    ...
    /** Prompt nhập Room ID rồi trả phòng tìm được từ DB (hoặc in lỗi nếu không có). */

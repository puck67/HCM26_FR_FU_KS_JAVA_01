...
export function TaskForm({ initialValues, onSubmit, onCancel, isSubmittingTask }: TaskFormProps) {
  return (
          ...
          <Form className="flex flex-col gap-4">
            ...
            <div className="flex gap-2 justify-end mt-4">
              ...
              <button
                ...
                className={`px-4 py-2 rounded font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 bg-blue-600 text-white hover:bg-blue-700 ${(isSubmitting || isSubmittingTask) ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {(isSubmitting || isSubmittingTask) ? 'Saving...' : 'Save'}
              </button>
            </div>
          </Form>
        )}

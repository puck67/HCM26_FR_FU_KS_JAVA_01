        ...
        <tbody>
          {data.map((item, rowIndex) => {
            ...
            return (
              ...
              >
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{(item as any).id}</td>
                {columns.map((col) => (
                  <td key={col} className="px-6 py-4 truncate max-w-xs">{String((item as any)[col])}</td>
                ))}
